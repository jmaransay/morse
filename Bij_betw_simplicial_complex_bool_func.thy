...
lemma ceros_of_boolean_input_in_set:
  ...
  shows "ceros_of_boolean_input (vec n (\<lambda>i. if i \<in> \<sigma> then False else True)) = \<sigma>"
...
definition bool_vec_from_simplice :: "nat set => (bool vec)"
  where "bool_vec_from_simplice \<sigma> = vec n (\<lambda>i. if i \<in> \<sigma> then False else True)"
...
lemma "bool_vec_from_simplice {0} = vec n (\<lambda>i. if i = (0::nat) then False else True)"
  unfolding bool_vec_from_simplice_def by auto

lemma "bool_vec_from_simplice {1,2} = 
    vec n (\<lambda>i. if i = (1::nat) \<or> i = 2 then False else True)"
  unfolding bool_vec_from_simplice_def by auto
...
definition bool_vec_set_from_simplice_set :: "nat set set => (bool vec) set"
  where "bool_vec_set_from_simplice_set K = {\<sigma>. (dim_vec \<sigma> = n) \<and> (\<exists>k\<in>K. \<sigma> = bool_vec_from_simplice k)}"
...
proof (unfold boolean_functions.monotone_bool_fun_def)
  ...
  next
    ...
    from True obtain k where r_def: "r = vec n (\<lambda>i. if i \<in> k then False else True)" 
      and k: "k \<in> K"
...
text\<open>The following lemma holds for every function x 
  (the premise on x being monotone can be omitted).
  On the other hand, we must assume that the boolean 
  vectors for which the property holds are the ones of 
  length n. Ideally, these property should be part of the
  definition of monotone boolean functions, but I did not
  find a suitable way to introduce it in locale boolean\_functions.\<close>
