...
proof -
  have fK1: "finite K1" and fK2: "finite K2" using f K1V K2V K2K1 unfolding powerset_def
    by (simp add: finite_subset)+
  show ?thesis
  using f v K1V K2V K2K1 fK1 fK2 proof (induct "card (K1 - K2)" arbitrary: K1 rule: less_induct)
  case less
  show ?case
  proof (cases "K1 = K2")
    case True
  ...
  next
    case True
    have jvK2: "join_vertex v K2 = {{v},{}}" unfolding True join_vertex_def join_def by simp
    show ?thesis
      unfolding jvK2
   ...
   proof (cases "a1 \<in> K1")
    ...
    proof (cases "a1 \<in> {{v}}")
     ...
     proof (cases "a1 \<in> K2")
      ...
      next
      case False hence "a1 \<in> {w. \<exists>s\<in>{{v}}. \<exists>s'\<in>K2. w = s \<union> s'}" 
       using a1 a1nv a1nK1 unfolding join_vertex_def join_def by simp
      hence "\<exists>s'\<in>K2. a1 = insert v s'" by simp
...
qed
  ...
  proof -
    ...
    moreover
    have "(K1 \<union> join_vertex v (K2 - {t, free_coface t K2}), K1 \<union> (join_vertex v K3)) \<in> collapses_rtrancl"
    ...
    qed
    ultimately show ?case using collapses_comp by presburger
  qed
qed
